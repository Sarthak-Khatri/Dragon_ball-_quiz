...
function initQuizPage() {
  ...
  function updateProgressDisplay() {
    ...
    if (progressDisplay) {
      progressDisplay.textContent = `${answeredCount} / ${quizData.length}`;
    }
  }
...
function initResultsPage() {
  ...
  if (shareBtn) {
    shareBtn.addEventListener('click', () => {
      const shareText = `I scored ${percentage}% on the Vegeta Elite Saiyan Quiz and earned the rank: ${rank}! Can you beat my score?`;
      if (navigator.share) {
        navigator.share({
          title: "Vegeta's Elite Saiyan Quiz Results",
          text: shareText,
